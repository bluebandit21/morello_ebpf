			     ...
			     void *data, struct drm_file *file_priv)
{
	...
	r_base = crtc->gamma_store;
	if (copy_from_user(r_base, uaddr_to_user_ptr(crtc_lut->red), size)) {
		ret = -EFAULT;
		goto out;
	}

	g_base = r_base + size;
	if (copy_from_user(g_base, uaddr_to_user_ptr(crtc_lut->green), size)) {
		ret = -EFAULT;
		goto out;
	}

	b_base = g_base + size;
	if (copy_from_user(b_base, uaddr_to_user_ptr(crtc_lut->blue), size)) {
		ret = -EFAULT;
		goto out;
	}
			     ...
			     void *data, struct drm_file *file_priv)
{
	...
	size = crtc_lut->gamma_size * (sizeof(uint16_t));
	r_base = crtc->gamma_store;
	if (copy_to_user(uaddr_to_user_ptr(crtc_lut->red), r_base, size)) {
		ret = -EFAULT;
		goto out;
	}

	g_base = r_base + size;
	if (copy_to_user(uaddr_to_user_ptr(crtc_lut->green), g_base, size)) {
		ret = -EFAULT;
		goto out;
	}

	b_base = g_base + size;
	if (copy_to_user(uaddr_to_user_ptr(crtc_lut->blue), b_base, size)) {
		ret = -EFAULT;
		goto out;
	}
out:
